...
impl TaskManager {
    ...
    fn get_already_run_time(&self) ->usize{
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
    ...
    fn get_syscall_times(&self) ->[u32;500]{
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].sys_call_times
    }
    ...
    fn mmap(&self,_start: usize, _len: usize, _port: usize)->isize{
        ...
        let current = innner.current_task;
        let mst = innner.tasks[current].memory_set.borrow_mut();
        mst.mmap(_start, _len, _port)
    }
    fn munmap(&self,_start: usize, _len: usize)->isize{
        let mut innner = self.inner.exclusive_access();
        let current = innner.current_task;
        let mst = innner.tasks[current].memory_set.borrow_mut();
        mst.munmap(_start, _len)
    }
}
